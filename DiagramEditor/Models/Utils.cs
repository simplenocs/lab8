	...
	public static class Utils {
		...
		private static string ToXMLHandler(object? obj, string level) {
			...
			if (obj is JsonElement @item) {
				...
				case JsonValueKind.False: return "_BOOL_false";
				case JsonValueKind.Null: return "null";
				}
			}
		...
		private static string YAML_ToJSONHandler(ref string yaml, ref int pos) {
			...
			case '-': {
				...
				while (true) {
					...
					if (First) First = false;
						...
						if (layer != layer3) {
							...
							if (!layer.StartsWith(layer3)) throw new Exception("Ошибка в layer");
					...
					if (layer2.Length < layer.Length) {
						if (!layer.StartsWith(layer2)) throw new Exception("Ошибка в layer");
						YAML_Log("Падение"); pos = saved_pos; break;
					}
					if (!layer2.StartsWith(layer)) throw new Exception("Ошибка в layer");
				...
				res.Append(']');
				YAML_Log("Список создан: " + res.ToString(), 2);
				return res.ToString(); }
			case '"':
			default: {
				pos--;
				...
				res.Append('}');
				YAML_Log("Словарь создан: " + res.ToString(), 2);
				return res.ToString(); }
			}
		}
