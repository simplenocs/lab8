...
namespace DiagramEditor.Models {
	public class Mapper {
		...
		private void RemoveItem(DiagramItem item) {
			items.Remove(item);
			var canv = (Canvas) (item.Parent ?? throw new Exception("Ошибка"));
		...
		public void Export(string type, Control canv) {
			if (type == "PNG") {
				...
				try { Utils.RenderToFile(canv, "../../../Export.png"); } catch (Exception e) { Log.Write("Ошибка: " + e); }
			...
			case "JSON":
				...
				if (json == null) { Log.Write("Не удалось экспортировать"); return; }
				File.WriteAllText("../../../Export.json", json);
				break;
			case "XML":
				var xml = Utils.Obj2xml(data);
				if (xml == null) { Log.Write("Не удалось экспортировать"); return; }
				File.WriteAllText("../../../Export.xml", xml);
				break;
			case "YAML":
				var yaml = Utils.Obj2yaml(data);
				if (yaml == null) { Log.Write("Не удалось экспортировать"); return; }
		...
		public DiagramItem[]? Import(string type, object? content = null) {
			...
			if (content == null) {
				if (!File.Exists("../../../" + name)) { Log.Write(name + " файл не найден"); return null; }
			...
			if (content is not Dictionary<string, object> @dict) { Log.Write("В начале " + name + " находится не словарь"); return null; }
			...
			if (items.Count > 0) {
				var canv = (Canvas) (items[0].Parent ?? throw new Exception("Ошибка"));
			...
			foreach (var obj in itemz) {
				...
				if (@item.Count != 5 ||
					...
					@item[3] is not int @w || @item[4] is not int @h) { Log.Write("Неправильное содержимое списка"); continue; }
			...
			foreach (var obj in @joins) {
				...
				if (@join.Count != 7 ||
					...
					@join[6] is not int @t) { Log.Write("Неправильное содержимое списка"); continue; }
		...
		private DiagramItem[]? GrandImport(string data) {
			...
			Log.Write("Неверный тип файла (JSON/XML/YAML)");
			return null;
		}
